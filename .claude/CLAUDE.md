...
- **No measured decimals in a comment.** A scanline figure (`0.01928`, `0.0032`) or an
  accuracy ratio (`reads 1.12`) is regenerated by every calibration run and is specific to
  one emulator build, so it is stale the moment someone re-measures. Write what survives
  instead: **instruction counts and relationships**, which come from the emitted code, not
  from a timing run — "one instruction, not six", "clamping is twice wrapping", "a little
  over at an even column and a little under at an odd one". Those explain the code AND stay
