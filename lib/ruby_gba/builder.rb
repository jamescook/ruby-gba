...
module RubyGBA
  ...
  class Builder
    ...
    # --- Randomness ---
    #
    # A deterministic pseudo-random number stream. There's no dice in the console
    # — "random" is really a hidden number the game keeps churning, so we own it
    # completely: the same seed always replays the same sequence, on every boot and
    # on every backend (the churn is plain multiply-and-add, which the interpreter
    # and the console agree on to the bit). That determinism is a feature — a game
    # can seed a constant and play out identically every time (handy for tests and
    # attract-mode demos), and only opt in to unpredictability by seeding from
    # something the player can't predict.
