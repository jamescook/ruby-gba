    ...
    module Backends
      class GBA
        ...
        class Statements
          ...
          # Clamp a variable into [min, max]: the variable held in its range, stored back — the
          # same two compare-and-maybe-replace steps as the new number (see
          # Expressions#eval_clamped).
          def emit_clamp(node)
            @lowering.value(Build.clamped(Build.var_ref(node.var), node.min, node.max))
            @primitives.store_var(ACC, node.var)
          end
