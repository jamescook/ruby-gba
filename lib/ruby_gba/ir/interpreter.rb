...
module RubyGBA
  module IR
    # Runs an IR::Node program directly in Ruby — the logic core of the reference
    # interpreter. It executes control flow, variable ops, and arithmetic against
    # an in-memory variable store, with NO hardware: drawing and input belong to a
    # later layer. Being able to just *run* a program and read the result is what
    # makes testing game logic cheap and headless, and it pins the IR's meaning
    # before the (harder) machine-code lowering has to reproduce it.
    #
    # It runs hand-built IR::Build trees today; it does not need the DSL to emit
    # IR yet.
    class Interpreter
      ...
      # Execute a program (or any statement node) until it ends naturally, hits a
      # `halt`, or exhausts the step budget — whichever comes first. Returns self
      # so callers can chain and then inspect variables.
      ...
      # True if run() stopped because it hit the step budget rather than a `halt`
      # or the natural end — i.e. it was still looping when we cut it off.
      ...
      # Register every func defined anywhere in the tree up front, so a `call` can
      # reach a func defined later in the program (a forward reference) — the same
      # resolve-names-first move the machine-code lowering makes with labels.
      ...
      def exec(node)
        ...
        when :func, :label, :label_ref
          # func bodies run only via `call`; a label is an inert marker here (the
          # structured IR has no goto that would jump to one).
          nil
        when :set
        ...
        when :wait_vblank
          ...
          raise ProgramError,
                "the interpreter core cannot execute #{node.kind.inspect} " \
