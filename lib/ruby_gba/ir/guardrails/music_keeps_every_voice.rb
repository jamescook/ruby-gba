      ...
      module Checks
        ...
        class MusicKeepsEveryVoice
          ...
          def detect(program)
            ...
            return [] if Tunes.mixer_voices(program) < Sound::MIXER_VOICES
          ...
          def message(song)
            ...
            "The song :#{song} has #{voices} parts that play an instrument. The mixer has #{voices} " \
              "voices, and the music keeps all #{voices} of them for the whole game. This game also " \
              "starts its own sounds with `play`. Those sounds never play. To fix this, give :#{song} " \
              "fewer parts that play an instrument."
          end
        end
      end
    end
