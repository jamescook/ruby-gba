      ...
      module Checks
        ...
        class SongNeedsFrames
          ...
          MESSAGE =
            ...
            "A song or a sound effect moves one step on every frame, so it needs frames to play on. " \
            "A frame ends when the game waits for the screen. With no wait there are no frames, and " \
            "the song or sound effect never plays. To fix this, put the game in a `game_loop`. A " \
            "`game_loop` waits for the screen on each pass."
