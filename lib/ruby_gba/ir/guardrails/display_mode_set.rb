  ...
  module IR
    module Guardrails
      ...
      module Checks
        ...
        # but never picks a display mode. On the Game Boy Advance nothing appears
        ...
        class DisplayModeSet
          ...
          # Any drawing op counts. `display` itself is categorized as a draw op,
          # so exclude it — it's the thing whose absence we're checking for.
          def draws?(program)
            program.each.any? { |node| node.category == :draw && node.kind != :display }
          end

          def display_set?(program)
            program.each.any? { |node| node.kind == :display }
          end

          # The safe fix: put a bitmap `display` at the very top, ahead of every
          # existing statement, so the screen is on before anything draws.
          def switch_screen_on(program)
            Build.program(Build.display(:bitmap), *program.children)
          end
        end
      end
    end
