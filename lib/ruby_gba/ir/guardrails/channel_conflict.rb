  ...
  module IR
    module Guardrails
      module Checks
        ...
        # song with *two square-wave parts* (a melody with a bass or harmony) needs a
        # second voice — and the only one left is the same one `beep` plays on. So while
        # such a song plays, a beep and the song's second part will keep cutting each
        # other off. That's silent and baffling if you don't know the console only has so
        ...
        # interrupting each other. A part that plays a recorded instrument sounds through
        # the mixer instead, so it never reaches the beep's voice.
