...
class TestSpriteOneFrameAtATime < Minitest::Test
  ...
  # A picture drawn in a 16-wide band starting at +from+ and see-through everywhere else, so the
  # build trims it to a 16-wide box standing +from+ along. Poses drawn from different columns
  # are trimmed by different amounts, which is the whole point: the sprite is told to stand a
  # different distance along for each of them, and a reader has to take that distance off again
  # before the console's number means the corner of the picture.
  def banded_art(number, from, size = 32)
  ...
  # Where the characters below are declared to stand, and where reading them back must say they
  # are, on every frame of their animation.
  STANDS_AT = 40

  # A character kept to one frame at a time whose poses do NOT all trim alike. The first is
  # drawn down the left of its canvas, the second in the middle, the next two are those two
  # REVERSED — which is how nearly every 2D game faces a character, and which trims from the
  # other side — and the rest fill their canvas. Every one of them is copied into the same room
  # in sprite memory, so the console's table says the same tile whichever is showing.
  def banded_screen
    program do |t|
      screen :tiled
      poses = (0...70).map { |n| :"step_#{n}" }
      poses.each_with_index do |name, n|
        data = n < 2 ? t.banded_art(n, n * 8) : t.frame_art(n)
        image name, width: 32, height: 32, data: data, transparent: true
      end
      showing = [poses[0], poses[1], mirror(poses[0]), mirror(poses[1]), *poses[2..]]
      sprite :walker, at: [STANDS_AT, STANDS_AT], frames: showing, rate: 1
      game_loop {}
    end
  end

  # A character bigger than any one picture the console draws, so every pose is cut into pieces —
  # and kept to one frame besides. Its pieces are separate rows of the console's table, each
  # trimmed on its own, so this is the case where being keyed by the pose has to hold per piece.
  def banded_giant_screen
    program do |t|
      screen :tiled
      poses = (0...40).map { |n| :"stride_#{n}" }
      poses.each_with_index do |name, n|
        data = n < 2 ? t.banded_art(n, n * 8, 48) : t.growing_art(n)
        image name, width: 48, height: 48, data: data, transparent: true
      end
      sprite :giant, at: [STANDS_AT, STANDS_AT], frames: poses, rate: 1
      game_loop {}
    end
  end

  # Read a character back over the frames of its animation: it must say it is where the game put
  # it every time, and the console's own number must really move between those frames, or the
  # test proves nothing.
  def assert_stands_still(prog, name:, sprite:, frames: 6)
    v = assert_emulator_loads_rom(assemble_rom(prog, name: name), frames: 2)
    raw = []
    frames.times do
  ...
  # THE BUG THIS PINS: the trim was looked up by the place in sprite memory a pose sits in, which
  # is the same place for every pose of a character kept to one frame. So every pose was given
  # whichever trim was written down last, and the position read back was silently out by the
  # difference for all the others — right on most frames of a cycle and wrong on a few, which is
  # the shape of mistake that costs a day.
  def test_a_character_kept_to_one_frame_reads_back_where_the_game_put_it
    assert_stands_still(banded_screen, name: "BAND", sprite: :walker)
  end

  def test_a_character_cut_into_pieces_and_kept_to_one_frame_reads_back_where_the_game_put_it
    assert_stands_still(banded_giant_screen, name: "GIANT", sprite: :giant)
  end
