...
# screen on for you.
class TestIRGuardrailScreenMode < Minitest::Test
  include RubyGBA::IR::Build

  Guardrails = RubyGBA::IR::Guardrails

  # A validator running only the screen-mode check, so these unit tests assert it
  ...
  def screen_validator
    Guardrails::Validator.new(checks: [Guardrails::Checks::ScreenModeSet.new])
  end

  def test_drawing_without_a_screen_mode_is_flagged
    report = screen_validator.run(program(pixel(10, 20, :red)), autofix: false)
    refute report.ok?
  ...
  def test_autofix_switches_the_screen_on_and_warns
    report = screen_validator.run(program(pixel(10, 20, :red)))
  ...
  def test_the_fixed_program_is_clean_on_a_second_pass
    fixed = screen_validator.run(program(pixel(10, 20, :red))).program
    report = screen_validator.run(fixed)
    assert report.ok?
    assert_empty report.findings
  end

  def test_a_program_that_already_sets_a_mode_is_left_alone
    report = screen_validator.run(program(screen(:bitmap), clear_screen(:blue)))
    assert report.ok?
    assert_empty report.findings
  end

  def test_a_program_that_never_draws_is_not_nagged
    report = screen_validator.run(program(set(:x, 1), loop_(add(:x, 1))), autofix: false)
    assert report.ok?
    assert_empty report.findings
  end

  def test_clear_screen_counts_as_drawing
    report = screen_validator.run(program(clear_screen(:blue)), autofix: false)
    refute report.ok?
  end
  ...
  def test_build_halts_on_a_draw_without_a_screen_mode
    err = StringIO.new
