...
# Tests for the native RubyGBAEmulator::Core surface — the thin libmgba mCore wrapper
...
class TestRubyGBAEmulatorCore < Minitest::Test
  include RubyGBAEmulatorTestSupport

  def test_loads_a_gba_rom_and_reports_its_shape
    core = RubyGBAEmulator::Core.new(red_rom)
  ...
  def test_title_and_maker_round_trip_from_the_header
    core = RubyGBAEmulator::Core.new(build_rom("HELLO", code: "THEL", maker: "42") do
      screen :bitmap
      clear_screen :blue
  ...
  def test_set_keys_accepts_a_bitmask_without_raising
    core = RubyGBAEmulator::Core.new(red_rom)
    core.set_keys(RubyGBAEmulator::KEY_RIGHT | RubyGBAEmulator::KEY_A)
    core.run_frame
    core.set_keys(0)
    core.run_frame
    core.destroy
  ...
  def test_key_constants_are_distinct_bits
    bits = %i[KEY_A KEY_B KEY_SELECT KEY_START KEY_RIGHT KEY_LEFT KEY_UP KEY_DOWN KEY_R KEY_L]
           .map { |k| RubyGBAEmulator.const_get(k) }
  ...
  def test_button_name_hash_is_frozen_and_complete
    map = RubyGBAEmulator::GBA_BTN_BITS
    assert_predicate map, :frozen?
    assert_equal RubyGBAEmulator::KEY_RIGHT, map[:right]
    assert_equal RubyGBAEmulator::KEY_START, map[:start]
  ...
  def test_xor_delta_and_changed_pixel_count
    ...
    delta = RubyGBAEmulator.xor_delta(a, b)
    # Two of the three 4-byte pixels differ.
    assert_equal 2, RubyGBAEmulator.count_changed_pixels(delta)
  end

  def test_xor_delta_rejects_mismatched_lengths
    assert_raises(ArgumentError) { RubyGBAEmulator.xor_delta("abcd", "abc") }
  end

  def test_bios_checksum_helper_and_constants_exist
    assert_kind_of Integer, RubyGBAEmulator::GBA_BIOS_CHECKSUM
    assert_kind_of Integer, RubyGBAEmulator::GBA_DS_BIOS_CHECKSUM
    # Checksumming 16 zero bytes is well-defined and non-raising.
    assert_kind_of Integer, RubyGBAEmulator.gba_bios_checksum("\x00".b * 16)
  end
  ...
  def test_rcheevos_is_absent_by_default
    refute RubyGBAEmulator.const_defined?(:RARuntime),
